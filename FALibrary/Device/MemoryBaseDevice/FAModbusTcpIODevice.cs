...
{
    public class FAModbusTcpIODevice : FAMemoryBaseDevice
    {
        private class ModbusTcpLibrary
        {
        }
        //--------------------------------------------------------------------------------------------------------
        public class ModbusTcp
        {
            ...
            public ModbusTcp(string ip, ushort port, bool no_sync_connection)
            {
            ...
            public void WriteMultipleCoils(ushort id, byte unit, ushort startAddress, ushort numBits, byte[] values, ushort numBytes)
            {
                if (numBytes > 250 || numBits > 2000)
                {
                    CallException(id, unit, fctWriteMultipleCoils, excIllegalDataVal);
        ...
        {
            string modbusDispMsg = "";
            // ------------------------------------------------------------------------
            // Identify requested data
            switch (ID)
            {
                case 1:
                    modbusDispMsg = "Read coils";
                    modbus_Data = values;
                    break;
                case 2:
                    modbusDispMsg = "Read discrete inputs";
                    modbus_Data = values;
                    break;
                case 3:
                    modbusDispMsg = "Read holding register";
                    modbus_Data = values;
                    break;
                case 4:
                    modbusDispMsg = "Read input register";
                    modbus_Data = values;
                    break;
                case 5:
                    modbusDispMsg = "Write single coil";
                    break;
                case 6:
                    modbusDispMsg = "Write multiple coils";
                    break;
                case 7:
                    modbusDispMsg = "Write single register";
                    break;
                case 8:
                    modbusDispMsg = "Write multiple register";
                    break;
            }
            Utility.Trace.WriteLine(this, "Device", modbusDispMsg);
        }
        ...
        private void MBmaster_OnException(ushort id, byte unit, byte function, byte exception)
        {
            ...
            switch (exception)
            {
                case ModbusTcp.excIllegalFunction: exc += "Illegal function!"; break;
                case ModbusTcp.excIllegalDataAdr: exc += "Illegal data adress!"; break;
                case ModbusTcp.excIllegalDataVal: exc += "Illegal data value!"; break;
                case ModbusTcp.excSlaveDeviceFailure: exc += "Slave device failure!"; break;
                case ModbusTcp.excAck: exc += "Acknoledge!"; break;
                case ModbusTcp.excGatePathUnavailable: exc += "Gateway path unavailbale!"; break;
                case ModbusTcp.excExceptionTimeout: exc += "Slave timed out!"; break;
                case ModbusTcp.excExceptionConnectionLost: exc += "Connection is lost!"; break;
                case ModbusTcp.excExceptionNotConnected: exc += "Not connected!"; break;
            }

            throw new Exception(exc + "Modbus slave exception[I/O Modbus통신 케이블 연결 확인하세요.]");
        }

        System.Diagnostics.Stopwatch stopwatch = new System.Diagnostics.Stopwatch();
        public override void Open()
        {
            modbusTcp = new ModbusTcp("192.168.200", 502, true);
            modbusTcp.OnResponseData    += new ModbusTcp.ResponseData(MBmaster_OnResponseData);
            modbusTcp.OnException       += new ModbusTcp.ExceptionData(MBmaster_OnException);
        ...
        public override void ReadWrite()
        {
            foreach (KeyValuePair<int, NodeInfo> nodeInfo in NodeInfoList)
            {
                ...
                {
                    ushort modbusID = 1;
                    int unit = nodeInfo.Value.ModuleID - 1;
                    int StartAddress = 0;
                    int Length = (nodeInfo.Value.InputSize + nodeInfo.Value.OutputSize) * 8;
                    modbusTcp.ReadCoils(modbusID, (byte)unit, (ushort)StartAddress, (ushort)Length);
                    ...
                    if (modbus_Data != null)
                    {
                        ...
                        {
                            if (modbus_Data.Length >= offset)
                                nodeInfo.Value.InputIO[offset] = modbus_Data[offset];
                        }
                        //------------------------------------------------------------------------------------------------------
                        // Output IO
                        StartAddress = nodeInfo.Value.InputSize;
                        for (int offset = 0; offset < nodeInfo.Value.OutputIO.Count(); offset++)
                        {
                            byte[] data = new byte[1];
                            data[0] = nodeInfo.Value.OutputIO[offset];
                            if (data[0] != modbus_Data[StartAddress + offset])
                            {
                                modbusID = 6;
                                int numBits = 8;
                                int StartBitAddress = (StartAddress * 8);
                                modbusTcp.WriteMultipleCoils(modbusID, (byte)unit, (ushort)StartBitAddress, (ushort)numBits, data, 1);
                            }
                        }
                        //------------------------------------------------------------------------------------------------------
                        //string szReadData = "";
                        //if (modbus_Data.Length >= 1) szReadData += string.Format("{0:x2} ", modbus_Data[0]);
                        //if (modbus_Data.Length >= 2) szReadData += string.Format("{0:x2} ", modbus_Data[1]);
                        //if (modbus_Data.Length >= 3) szReadData += string.Format("{0:x2} ", modbus_Data[2]);
                        //System.Diagnostics.Trace.WriteLine("modbusTcp Read(len=" + Length.ToString() +
                        //    "), ScanTime(msec):" + stopwatch.ElapsedMilliseconds + ", modbus_Data=" + szReadData);
                        //------------------------------------------------------------------------------------------------------
                    }
                    stopwatch.Restart();
                }
