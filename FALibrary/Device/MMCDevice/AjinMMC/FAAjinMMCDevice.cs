...
namespace FALibrary.Device.MMCDevice.AjinMMC
{
    public class FAAjinMMCDevice : FAMMCDevice
    {
        private class AjinLibrary
        {
            ...
            public class CAXM
            {
                ...
                [DllImport("AXL.dll")]
                public static extern uint AxmMotSetTorqueLimitAtPos(int lAxisNo, double dPluseDirTorqueLimit,
                                                        double dMinusDirTorqueLimit, double dPosition, long lTarget);

                /////해당축의 토크값을 추가한다
                [DllImport("AXL.dll")]
                public static extern uint AxmStatusReadTorque(int lAxisNo, ref double pdTorque);
        ...
        public string MotionParamPath { get; set; }
        ...
        public override void Open()
        {
            ...
            string path = AppDomain.CurrentDomain.BaseDirectory + MotionParamPath;

            if (System.IO.File.Exists(path) == true)
                AjinLibrary.CAXM.AxmMotLoadParaAll(path);
            else
                throw new Exception(this.Name + " not found " + path);

            //edge-bang-2024-06-06
            //uint nret;
            //uint uVLIM = 0xFFFFFFFF;
            //int lTQREF = 1;

            //nret = AjinLibrary.CAXM.AxmM3ServoTrqctrl(8, uVLIM, lTQREF);
            //nret = AjinLibrary.CAXM.AxmM3ServoTrqctrl(9, uVLIM, lTQREF);
            //nret = AjinLibrary.CAXM.AxmM3ServoTrqctrl(10, uVLIM, lTQREF);
            //nret = AjinLibrary.CAXM.AxmM3ServoTrqctrl(11, uVLIM, lTQREF);
            //->
        }
        ...
        public override int MovePos(ushort axisNo, double position, ushort speedMode, double startSpeed, double moveSpeed, double accelTime, double decTime)
        {
            ...
            AjinLibrary.CAXM.AxmMotSetAbsRelMode(axisNo, 0); // set absolute pos mode


            AjinLibrary.CAXM.AxmMoveStartPos(axisNo, position, moveSpeed, accel, decel);

            //{
            //    AjinLibrary.CAXM.AxmMovePos(axisNo, position, moveSpeed, accel, decel);
            //}
            return 0;
        }
        ...
        public override void MoveIncPos(ushort axisNo, int position, ushort speedMode, double startSpeed, double moveSpeed, double accelTime, double decTime)
        {
            ...
            AjinLibrary.CAXM.AxmMotSetAbsRelMode(axisNo, 1); // set offset pos mode


            //if (axisNo == 8 || axisNo == 9 || axisNo == 10 || axisNo == 11)
        ...
        public override void MoveVelocity(ushort axisNo, double startSpeed, double moveSpeed, double accelTime, double decTime)
        {
            ...
            // AjinLibrary.CAXM.AxmMoveVel(axisNo, moveSpeed, accel, decel);
            AjinLibrary.CAXM.AxmMoveVel(axisNo, moveSpeed, 3000.0, 3000.0);
        }

        public override void MoveSpeedOverrideVelocity(ushort axisNo, double dOverrideVelocity)
        {
            AjinLibrary.CAXM.AxmOverrideVel(axisNo, dOverrideVelocity);
        }

        public override bool IsMotionDone(ushort axisNo)
        {
            if (IsRunFlag(axisNo) == false &&
                IsHomeFlag(axisNo) == false) return true;
        ...
        {
            AjinLibrary.CAXM.AxmHomeSetMethod(axisNo, homeDir,
                homeMode,
                (int)AjinLibrary.AXT_BOOLEAN.FALSE,
                1000.0,
                offset);

            double accel = moveSpeed / (accelTime * 0.001);

            AjinLibrary.CAXM.AxmHomeSetVel(axisNo, moveSpeed, moveSpeed * 0.1, moveSpeed * 0.05, moveSpeed * 0.05, accel, accel * 0.1);

            AjinLibrary.CAXM.AxmHomeSetStart(axisNo);
        }

        public override void Stop(ushort axisNo)
        {
            AjinLibrary.CAXM.AxmMoveEStop(axisNo);
            //AjinLibrary.CAXM.AxmMoveSStop(axisNo);
        }
        ...
        {
            uint status = 0;
            AjinLibrary.CAXM.AxmHomeReadSignal(axisNo, ref status);
            if (status == (uint)AjinLibrary.AXT_MOTION_SIGNAL_LEVEL.ACTIVE)
                return true;
        ...
        {
            uint status = 0;
            AjinLibrary.CAXM.AxmSignalGetZphaseLevel(axisNo, ref status);
            if (status == (uint)AjinLibrary.AXT_MOTION_LEVEL_MODE.HIGH)
                return true;
        ...
        {
            uint status = 0;
            AjinLibrary.CAXM.AxmSignalReadStop(axisNo, ref status);
            if (status == (uint)AjinLibrary.AXT_BOOLEAN.TRUE)
                return true;
        ...
        {
            uint status = 0;
            AjinLibrary.CAXM.AxmSignalReadInpos(axisNo, ref status);
            if (status == (uint)AjinLibrary.AXT_BOOLEAN.TRUE)
                return true;
        ...
        {
            uint status = 0;
            AjinLibrary.CAXM.AxmSignalReadServoAlarm(axisNo, ref status);
            if (status == (uint)AjinLibrary.AXT_BOOLEAN.TRUE)
                return true;
        ...
        //{
        //    AjinLibrary.CAXM.AxmStatusSetReadServoLoadRatio(lAxisNo, 0);
        ...
        //    AjinLibrary.CAXM.AxmStatusReadServoLoadRatio(lAxisNo, ref dpMonitorValue);
        ...
        public override bool IsPositiveLimit(ushort axisNo)
        {
            ...
            positive = 0;
            negative = 0;
            AjinLibrary.CAXM.AxmSignalReadLimit(axisNo, ref positive, ref negative);

            if (positive == (uint)AjinLibrary.AXT_BOOLEAN.TRUE)
                return true;
        ...
        public override bool IsNegativeLimit(ushort axisNo)
        {
            ...
            if (negative == (uint)AjinLibrary.AXT_BOOLEAN.TRUE)
                return true;
        ...
        {
            uint status = 0;
            AjinLibrary.CAXM.AxmStatusReadInMotion(axisNo, ref status);

            if (status == (uint)AjinLibrary.AXT_BOOLEAN.TRUE) return true;
        ...
        public override bool IsErrorFlag(ushort axisNo)
        {
            ...
            if (status == (uint)AjinLibrary.AXT_MOTION_SIGNAL_LEVEL.ACTIVE) return true;
        ...
        {
            uint status = 0;
            AjinLibrary.CAXM.AxmStatusReadMotion(axisNo, ref status);
            if (status == (uint)AjinLibrary.AXT_MOTION_QIDRIVE_STATUS.QIDRIVE_STATUS_7)
                return true;
        ...
        {
            uint use = 0;
            AjinLibrary.CAXM.AxmSignalIsServoOn(axisNo, ref use);


            if (use == (uint)AjinLibrary.AXT_USE.ENABLE) return true;
        ...
        {
            AjinLibrary.CAXM.AxmSignalServoAlarmReset(axisNo, (uint)AjinLibrary.AXT_BOOLEAN.TRUE);
            System.Threading.Thread.Sleep(200);
            AjinLibrary.CAXM.AxmSignalServoAlarmReset(axisNo, (uint)AjinLibrary.AXT_BOOLEAN.FALSE);
        }

        public override void ServoOn(ushort axisNo)
        {
            AjinLibrary.CAXM.AxmSignalServoOn(axisNo, (uint)AjinLibrary.AXT_USE.ENABLE);

            if (axisNo == 8 || axisNo == 9 || axisNo == 10 || axisNo == 11 || axisNo == 12)
            {
                long ret = AjinLibrary.CAXM.AxmStatusSetReadServoLoadRatio(axisNo, 2);
        ...
        {
            AjinLibrary.CAXM.AxmSignalServoOn(axisNo, (uint)AjinLibrary.AXT_USE.DISABLE);
        }

        //jbpark_2020.07.07 MOTOR DIO ON/OFF
        // lBitNo : Bit Number(0 - 4)
        public override void MIO_OnOff(ushort axisNo, ushort nBitNo, bool bOn)
        {
            AjinLibrary.CAXM.AxmSignalWriteOutputBit(axisNo, nBitNo, (uint)(bOn ? 1 : 0));
        }
        public override double GetCommandPos(ushort axisNo)
        {
            double pos = 0;
            AjinLibrary.CAXM.AxmStatusGetCmdPos(axisNo, ref pos);
            return (double)pos;
        }
        ...
        {
            double pos = 0;
            AjinLibrary.CAXM.AxmStatusGetActPos(axisNo, ref pos);
            return (double)pos;
        }
        ...
        {
            double vel = 0;
            AjinLibrary.CAXM.AxmStatusReadVel(axisNo, ref vel);
            return (int)vel;
        }

        public override void ResetCommandPos(ushort axisNo)
        {
            AjinLibrary.CAXM.AxmStatusSetCmdPos(axisNo, 0);
        }

        public override void ResetActualPos(ushort axisNo)
        {
            AjinLibrary.CAXM.AxmStatusSetActPos(axisNo, 0);
        }

        public override void SetLinkMode(ushort masterAxis, ushort slaveAxis, double ratio)
        {
            AjinLibrary.CAXM.AxmLinkSetMode(masterAxis, slaveAxis, ratio);
        }

        public override void ResetLinkMode(ushort masterAxis)
        {
            AjinLibrary.CAXM.AxmLinkResetMode(masterAxis);
        }

        public override bool IsHomeOk(ushort axisNo)
        {
            uint mainStepNo = 0, homeStepNo = 0;
            AjinLibrary.CAXM.AxmHomeGetRate(axisNo, ref mainStepNo, ref homeStepNo);
            if (homeStepNo >= 100)
            {
                uint homeResult = 0;
                AjinLibrary.CAXM.AxmHomeGetResult(axisNo, ref homeResult);
                if (homeResult == (uint)AjinLibrary.AXT_MOTION_HOME_RESULT.HOME_SUCCESS)
                    return true;
            }
        ...
        {
            var arr = info.OrderBy(x => x.AxisNo).ToArray();
            int len = info.Length;
            int[] axisNo = new int[len];
            double[] positions = new double[len];
            double[] maxVelocity = new double[len];
            double[] maxAccel = new double[len];
            double[] maxDecel = new double[len];

            for (int i = 0; i < len; i++)
            {
                axisNo[i] = arr[i].AxisNo;
                positions[i] = arr[i].Positions;
                maxVelocity[i] = arr[i].MaxVelocity;
                maxAccel[i] = arr[i].MaxVelocity / (arr[i].MaxAccel * 0.001);
                maxDecel[i] = arr[i].MaxVelocity / (arr[i].MaxDecel * 0.001);
            }

            AjinLibrary.CAXM.AxmMoveStartMultiPos(len, ref axisNo[0], ref positions[0], ref maxVelocity[0], ref maxAccel[0], ref maxDecel[0]);
        }
        ...
        {
            uint nret = AjinLibrary.CAXM.AxmMoveStartTorque(lAxisNo, dTorque, dVel, uAccFilterSel, uGainSel, uSpdLoopSel);
            int k = 0;

            //double dPluseDirTorqueLimit = 3000 ;
            //             double dMinusDirTorqueLimit = 3000;
            //double dPosition =1000 ;

            //long lTarget = 1 ;

            //uint nret1  = AjinLibrary.CAXM.AxmMotSetTorqueLimitAtPos(lAxisNo,  dPluseDirTorqueLimit,
            //                                             dMinusDirTorqueLimit,  dPosition,  lTarget);
        ...
        {
            AjinLibrary.CAXM.AxmMoveTorqueStop(lAxisNo, uMethod);
        }
        ...
        public override void MotSetTorqueLimit(int lAxisNo, double dbPluseDirTorqueLimit, double dbMinusDirTorqueLimit)
        {
            ...
            double ret = AjinLibrary.CAXM.AxmMotSetTorqueLimit(lAxisNo, dbPluseDirTorqueLimit, dbMinusDirTorqueLimit);
        ...
        public override void MotGetTorqueLimit(int lAxisNo, ref double dbpPluseDirTorqueLimit, ref double dbpMinusDirTorqueLimit)
        {
            ...
            double ret = AjinLibrary.CAXM.AxmMotGetTorqueLimit(lAxisNo, ref dbpPluseDirTorqueLimit, ref dbpMinusDirTorqueLimit);
        ...
        {
            AjinLibrary.CAXM.AxmM3ServoSetTorqProfile(lCoord, lAxisNo, TorqueSign, dwVLIM, dwProfileMode, dwStdTorq, dwStopTorq);
        }
        ...
        {
            AjinLibrary.CAXM.AxmM3ServoGetTorqProfile(lCoord, lAxisNo, ref lpTorqueSign, ref updwVLIM, ref upProfileMode, ref upStdTorq, ref upStopTorq);
        }
        public override void StatusSetServoMonitor(int nAxisNo, uint uSelMon, double dActionValue, uint uAction)
        {
            AjinLibrary.CAXM.AxmStatusSetServoMonitor(nAxisNo, uSelMon, dActionValue, uAction);
        }
        public override void StatusGetServoMonitor(int nAxisNo, uint uSelMon, ref double dpActionValue, ref uint upAction)
        {
            AjinLibrary.CAXM.AxmStatusGetServoMonitor(nAxisNo, uSelMon, ref dpActionValue, ref upAction);
        }
        public override void StatusSetServoMonitorEnable(int nAxisNo, uint uEnable)
        {
            AjinLibrary.CAXM.AxmStatusSetServoMonitorEnable(nAxisNo, uEnable);
        }
        public override void StatusGetServoMonitorEnable(int nAxisNo, ref uint upEnable)
        {
            AjinLibrary.CAXM.AxmStatusGetServoMonitorEnable(nAxisNo, ref upEnable);
        }
        ...
        {
            AjinLibrary.CAXM.AxmStatusReadServoMonitorFlag(nAxisNo, uSelMon, ref upMonitorFlag, ref dpMonitorValue);
        }
        public override void StatusReadServoMonitorValue(int nAxisNo, uint uSelMon, ref double dpMonitorValue)
        {
            AjinLibrary.CAXM.AxmStatusReadServoMonitorValue(nAxisNo, uSelMon, ref dpMonitorValue);
        }
        public override void StatusSetReadServoLoadRatio(int lAxisNo, uint dwSelMon)
        {
            AjinLibrary.CAXM.AxmStatusSetReadServoLoadRatio(lAxisNo, dwSelMon);
        }
        public override void StatusReadServoLoadRatio(int lAxisNo, ref double dpMonitorValue)
        {
            //double test = 0;
            long ret = AjinLibrary.CAXM.AxmStatusReadServoLoadRatio(lAxisNo, ref dpMonitorValue);
            //long ret2 = AjinLibrary.CAXM.AxmStatusReadTorque(lAxisNo, ref test);
        }
        //->add

        public override void M3ServoCoordinatesSet(int lAxisNo, uint dwPosData, uint dwPos_sel, uint dwRefe)
        {
            AjinLibrary.CAXM.AxmM3ServoCoordinatesSet(lAxisNo, dwPosData, dwPos_sel, dwRefe);
        }

        public override void M3ServoBreakOn(int lAxisNo)
        {
            AjinLibrary.CAXM.AxmM3ServoBreakOn(lAxisNo);
        }
        public override void M3ServoBreakOff(int lAxisNo)
        {
            AjinLibrary.CAXM.AxmM3ServoBreakOff(lAxisNo);
        }
        public override void M3ServoConfig(int lAxisNo, uint dwCfMode)
        {
            AjinLibrary.CAXM.AxmM3ServoConfig(lAxisNo, dwCfMode);
        }
        public override void M3ServoSensOn(int lAxisNo)
        {
            AjinLibrary.CAXM.AxmM3ServoSensOn(lAxisNo);
        }
        public override void M3ServoSensOff(int lAxisNo)
        {
            AjinLibrary.CAXM.AxmM3ServoSensOff(lAxisNo);
        }
        public override void M3ServoSmon(int lAxisNo)
        {
            AjinLibrary.CAXM.AxmM3ServoSmon(lAxisNo);
        }
        public override void M3ServoGetSmon(int lAxisNo, ref uint pbParam)
        {
            AjinLibrary.CAXM.AxmM3ServoGetSmon(lAxisNo, ref pbParam);
        }
        public override void M3ServoSvOn(int lAxisNo)
        {
            AjinLibrary.CAXM.AxmM3ServoSvOn(lAxisNo);
        }
        public override void M3ServoSvOff(int lAxisNo)
        {
            AjinLibrary.CAXM.AxmM3ServoSvOff(lAxisNo);
        }
        public override void M3ServoInterpolate(int lAxisNo, uint dwTPOS, uint dwVFF, uint dwTFF, uint dwTLIM)
        {
            AjinLibrary.CAXM.AxmM3ServoInterpolate(lAxisNo, dwTPOS, dwVFF, dwTFF, dwTLIM);
        }
        public override void M3ServoPosing(int lAxisNo, uint dwTPOS, uint dwSPD, uint dwACCR, uint dwDECR, uint dwTLIM)
        {
            AjinLibrary.CAXM.AxmM3ServoPosing(lAxisNo, dwTPOS, dwSPD, dwACCR, dwDECR, dwTLIM);
        }
        public override void M3ServoFeed(int lAxisNo, uint dwSPD, uint dwACCR, uint dwDECR, uint dwTLIM)
        {
            AjinLibrary.CAXM.AxmM3ServoFeed(lAxisNo, dwSPD, dwACCR, dwDECR, dwTLIM);
        }
        ...
        {
            AjinLibrary.CAXM.AxmM3ServoExFeed(lAxisNo, dwSPD, dwACCR, dwDECR, dwTLIM, dwExSig1, dwExSig2);
        }
        ...
        {
            AjinLibrary.CAXM.AxmM3ServoExPosing(lAxisNo, dwTPOS, dwSPD, dwACCR, dwDECR, dwTLIM, dwExSig1, dwExSig2);
        }
        ...
        {
            AjinLibrary.CAXM.AxmM3ServoZret(lAxisNo, dwSPD, dwACCR, dwDECR, dwTLIM, dwExSig1, dwExSig2, bHomeDir, bHomeType);
        }
        public override void M3ServoVelctrl(int lAxisNo, uint dwTFF, uint dwVREF, uint dwACCR, uint dwDECR, uint dwTLIM)
        {
            AjinLibrary.CAXM.AxmM3ServoVelctrl(lAxisNo, dwTFF, dwVREF, dwACCR, dwDECR, dwTLIM);
        }
        public override void M3ServoTrqctrl(int lAxisNo, uint dwVLIM, int lTQREF)
        {
            AjinLibrary.CAXM.AxmM3ServoTrqctrl(lAxisNo, dwVLIM, lTQREF);
        }
        public override void M3ServoGetParameter(int lAxisNo, uint wNo, uint bSize, uint bMode, ref uint pbParam)
        {
            AjinLibrary.CAXM.AxmM3ServoGetParameter(lAxisNo, wNo, bSize, bMode, ref pbParam);
        }
        public override void M3ServoSetParameter(int lAxisNo, uint wNo, uint bSize, uint bMode, ref uint pbParam)
        {
            AjinLibrary.CAXM.AxmM3ServoSetParameter(lAxisNo, wNo, bSize, bMode, ref pbParam);
        }
        //->
    }
}
