...
{
    public class FAPassModule : FAInlineModule
    {
        #region Sequences
        ...
        #region Parameters
        private bool _cancelProduct;
        /// <summary>
        /// Process가 진행된 후 NextModule로 이송하지 않고
        /// 다음 제품을 받도록 준비한다.
        /// </summary>
        [FAAttribute("Parameters")]
        public bool CancelProduct
        {
            get { return _cancelProduct; }
            set
            {
                if (_cancelProduct == value) return;
                _cancelProduct = value;
                NotifyPropertyChanged("CancelProduct");
            }
        }

        /// <summary>
        /// FrontModule에서 제품을 이송받지 않고 바로 Process로 진입한다.
        /// 제품이 이미 존재하는 경우 등을 위해 추가
        /// </summary>
        ...
        protected virtual void MakeLoop()
        {
            ...
            seq.AddStep("ConfirmCancelProduct").StepIndex = seq.AddItem(ConfirmCancelProduct);
        ...
        private void ConfirmCancelProduct(FASequence actor, TimeSpan time)
        {
            if (CancelProduct)
                actor.NextStep("ActionOfTerminateOneCycle");
            else
                actor.NextStep("ConfirmNextModuleExist");
        }
